...
class Sprites:
    def __init__(self) -> None:
        ...
        self.myFont = ImageFont.truetype('Base/TicketToRide/FreeMonoBoldOblique.ttf',size = 40*RATIO)
        self.myFont_end = ImageFont.truetype('Base/TicketToRide/FreeMonoBoldOblique.ttf',size = 20*RATIO)

        self.font2 = ImageFont.truetype('Base/TicketToRide/FreeMonoBoldOblique.ttf',size = 60*RATIO)
        self.PLAYER_TRAIN_CARD_COOR = np.array([[1400, 0],
                                                [1400, 80],
                                                [1400, 160],
                                                [1400, 240],
                                                [1400, 320],
        ...
        self.COUNT_PLAYER_TRAIN_CARD_COOR = np.array([[1530, 20],
                                                    [1530, 100],
                                                    [1530, 180],
                                                    [1530, 260],
                                                    [1530, 340],
        ...
        self.COUNT_PLAYER_TRAIN_CARD_TUNNEL_COOR = np.array([[1380, 20],
                                                    [1380, 100],
                                                    [1380, 180],
                                                    [1380, 260],
                                                    [1380, 340],
        ...
        self.CARD_TRAIN_CARD_BOARD = np.array([[1200, 0],
                                                [1200, 80],
                                                [1200, 160],
                                                [1200, 240],
                                                [1200, 320],
        ...
        self.CARD_TRAIN_CAR_BOARD_DARE = np.array([[1200, 480],
                                                [1200, 560],
                                                [1200, 640]])*RATIO

        self.ROUTE_CARD_BOARD_COOR = np.array([[1200, 480],
                                                [1200, 560],
                                                [1200, 640],
                                                [1200, 720]])*RATIO

        self.ROUTE_CARD_COOR = np.array([[0, 800],
                                        [80, 800],
                                        [160, 800],
                                        [240, 800],
                                        [320, 800],
        ...
        for value in card_values:
            self.cards.append(Image.open(IMG_PATH+f"trainCard_{value}.png").resize(CARD_SIZE))
        self.cards.append(Image.open(IMG_PATH+f"trainCard_down.png").resize(CARD_SIZE))

        route_id = np.arange(46)
        self.route_card = []
        for route in route_id:
            self.route_card.append(Image.open(IMG_PATH+f"/images_route/{route}.png").resize(ROUTE_SIZE))
...
def get_state_image(state=None):
    ...
    for id_player in range(5):
        player_road = np.where(all_player_road[101*id_player : 101*(id_player + 1)])[0]
    ...
    #chồng thẻ route úp
    if state[657]:
        background.paste(sprites.route_card[-1], tuple(sprites.ROUTE_CARD_BOARD_COOR[-1]))
...
def draw_route_cards_player(bg, route_cards):
    ...
    for i in range(n):
        ...
        bg.paste(card, (sprites.ROUTE_CARD_COOR[i][0], sprites.ROUTE_CARD_COOR[i][1]), )
...
def draw_player_road(roads, player_id, draw):
    list_color = [(100,255,100), (100,255,200), (0,0,0), (100,60,100), (0,0,255)]
...
def draw_other(bg, draw, state):
    ...
    if state[659]:
        ...
        for i in range(4):
            draw.text(tuple(sprites.IN4_END[i]), f"{all_text[i]}{all_in4[i]}", (0,0,0), font= sprites.myFont_end)
    else:
        all_text = ['score: ', 'Phase: ', 'last_player: ']
        draw.text(tuple(sprites.IN4_END[0]), f"{all_text[0]}{player_score}", (0,0,0), font= sprites.myFont)
        draw.text(tuple(sprites.IN4_END[1]), f"{all_text[1]}{phase}", (0,0,0), font= sprites.myFont)
        if np.sum(last_player) > 0:
            draw.text(tuple(sprites.IN4_END[2]), f"{all_text[2]}{last_player}", (0,0,0), font= sprites.myFont)
        if phase == 2:
            type_builds = state[643:652]
            for t_build in range(9):
                if type_builds[t_build] != 0:
                    draw.text(sprites.COUNT_PLAYER_TRAIN_CARD_TUNNEL_COOR[t_build], f"C", (100,255,100), font= sprites.myFont)
            ...
            for i in range(len(route_get)):
                ...
                bg.paste(sprites.route_card[route_card_get_id], tuple(sprites.ROUTE_CARD_BOARD_COOR[i]))

            draw.text(tuple(sprites.ROUTE_CARD_BOARD_COOR[-1]), f"drop:{number_drop}", (0,0,0), font= sprites.myFont)
            ...
            for t_build in range(9):
                if card_build_tunnel[t_build] != 0:
                    draw.text(sprites.COUNT_PLAYER_TRAIN_CARD_TUNNEL_COOR[t_build], f"{card_build_tunnel[t_build]}", (100,255,100), font= sprites.myFont)
    ...
    #print number_train
    draw.text(tuple(sprites.PLAYER_NUMBER_TRAIN), f"Train:{int(state[667])}", (0,0,0), font= sprites.myFont)
