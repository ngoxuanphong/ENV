...
action_annotations = ["Nope", "Attack", "Skip", "Favor", "Shuffle", "See the future", "Draw card", "Play doublecard", "Play triplecard", "Play five kinds of card", "Not Nope"] + [f"Choose player {i+1} to rob" for i in range(4)] + [f'Choose "{params.card_order[i]}"-card to give' for i in range(12)] + [f'Ask about "{params.card_order[i]}"-card' for i in range(12)] + [f'Choose "{params.card_order[i]}"-card from Discard pile' for i in range(12)] + [f'Discard "{params.card_order[i]}"-card' for i in range(11)]
