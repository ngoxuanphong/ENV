...
class Params:
    def __init__(self) -> None:
        ...
        self.fontFold = ImageFont.truetype('ImageFonts/FreeMonoBoldOblique.ttf',size = 60)
...
def get_state_image(state=None):
    #mảng tọa độ in ảnh
    arr_coordinate_card = np.array([[400,  720], [1000,  720], [1300,  550], [1300,  300], [1050,  100], [ 700,  100], [ 350,  100], [ 150,  300], [ 150,  550]])
    arr_coordinate_button_dealer = np.array([[440,  680], [1040,  680], [1270,  570], [1270,  330], [1070,  200], [ 740,  200], [ 390,  200], [ 250,  330], [ 250,  570]])
    arr_coordinate_small = np.array([[400,  680], [1000,  680], [1270,  540], [1270,  300], [1040,  200], [ 710,  200], [ 360,  200], [ 250,  300], [ 250,  540]])
    arr_coordinate_big = np.array([[480,  680], [1080,  680], [1270,  600], [1270,  360], [1100,  200], [ 770,  200], [ 420,  200], [ 250,  360], [ 250,  600]])
    arr_coordinate_status = np.array([[420,  750], [1020,  750], [1320,  560], [1320,  320], [1080,  120], [ 720,  120], [ 370,  120], [ 180,  320], [ 180,  560]])
    arr_coordinate_chip_else = np.array([[420,  830], [1020,  830], [1380,  560], [1380,  330], [1080,  70], [ 720,  70], [ 370,  70], [ 80,  330], [ 80,  560]])
    arr_coordinate_chip_in_pot = np.array([[320,  800], [920,  800], [1320,  520], [1320,  270], [980,  120], [ 620,  120], [ 270,  120], [ 180,  270], [ 180,  520]])
    ...
    for i in range(9):
        ...
        draw_cards(background, player_i_card, arr_coordinate_card[i][0], arr_coordinate_card[i][1])
        draw.text((arr_coordinate_chip_else[i][0], arr_coordinate_chip_else[i][1]),f"{int(state[468+i])}",(255,0,0), font= myFont)

    open_card_coodinate = np.array([[640, 400], [720, 400], [800, 400], [880, 400], [960, 400]])
    for i in range(len(open_card)):
        draw_cards(background, open_card[i:i+1], open_card_coodinate[i][0], open_card_coodinate[i][1])
    #DEALER, SMALL, BIG, STATUS
    if np.sum(player_in) > 1 and state[513] < 100:
        dealer = np.where(state[495 : 504])[0][0]
        draw.text((arr_coordinate_button_dealer[dealer][0], arr_coordinate_button_dealer[dealer][1]),f"D",(255,255,255), font= myFont)
        check_BS = 0
        id_player = (dealer + 1) % 9
        while check_BS < 2:
            if player_in[id_player] == 1:
                if check_BS == 0:
                    draw.text((arr_coordinate_small[id_player][0], arr_coordinate_small[id_player][1]),f"S",(255,0,0), font= myFont)
                else:
                    draw.text((arr_coordinate_big[id_player][0], arr_coordinate_big[id_player][1]),f"B",(255,255,0), font= myFont)
                check_BS += 1
            id_player = (id_player + 1) % 9
        
        for i in range(9):
            if arr_player_status[i] == 0:
                draw.text((arr_coordinate_status[i][0], arr_coordinate_status[i][1]),f"F",(255,0,0), font= fontFold)
            else:
                draw.text((arr_coordinate_chip_in_pot[i][0], arr_coordinate_chip_in_pot[i][1]),f"{int(state[477 + i])}",(255,165,0), font= myFont)
