...
class Sprites:
    def __init__(self) -> None:
        self.font=ImageFont.truetype("ImageFonts/FreeMonoBoldOblique.ttf", int(120/tl))
        self.font_max=ImageFont.truetype("ImageFonts/FreeMonoBoldOblique.ttf", int(200/tl))
        self.font2=ImageFont.truetype("ImageFonts/FreeMonoBoldOblique.ttf", int(60/tl))
        ...
        for d_x in range(5):
            if d_y == 3:
                if d_x < len(list_id_noble):
                    background.paste(sprites.list_img_card_noble[list_id_noble[d_x]], (x, int(y*2.1)))
            else:
                if d_x == 0:
                    ...
                    if list_id_normal[id_card_in_list] != -1:
                        background.paste(sprites.list_img_card[list_id_normal[id_card_in_list]], (x, y))
            x += CARD_SIZE[0] + _d_
        y -= CARD_SIZE[1] + _d_
...
def draw_tokens_image(im, 
    ...
    ImageDraw.Draw(im).text((x, int(BOARD_SIZE[1]*0.03)), f'Nguyên liệu mặc định:', fill= 'white', font = sprites.font2)
    for i in range(5):
        im.paste(sprites.list_img_token[i], (x, y))
...
def draw_score(im, state):
    ...
    for agent in range(4):
        ImageDraw.Draw(im).text((x, y - int(BOARD_SIZE[1]*0.03)), f'Điểm:', fill= 'white', font = sprites.font2)
        ImageDraw.Draw(im).text((x, y), f'{scores[agent]}', fill= 'white', font = sprites.font_max)
        if agent == 0:
            ImageDraw.Draw(im).text((x, y + int(BOARD_SIZE[1]*0.11)), f'YOU', fill= 'white', font = sprites.font)
        y += int(im.size[1]/4)
...
def get_main_player_state(env_components: Env_components, list_agent, list_data, action=None):
    if not action is None:
        env_components.env, env_components.lv1, env_components.lv2, env_components.lv3 = _env.stepEnv(action, env_components.env, env_components.lv1, env_components.lv2, env_components.lv3, env_components.list_action)
    ...
    if env_components.winner == 0: #no winner
        ...
        for p_idx in range(4):
            if p_idx != my_idx:
                env[100] = p_idx
                _state = _env.getAgentState(env, env_components.lv1, env_components.lv2, env_components.lv3)
                _state[220] = 1
