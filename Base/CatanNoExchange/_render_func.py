...
class Sprites:
    def __init__(self) -> None:
        ...
        self.largest_army = Image.open(IMG_PATH+"icon_largest_army_highlight.png").resize((40, 40)).convert("RGBA")
        self.longest_road = Image.open(IMG_PATH+"icon_longest_road_highlight.png").resize((40, 40)).convert("RGBA")
...
def init_map(state):
    ...
    for i in range(19):
        if probs[i] != 1:
            sprites.background.paste(sprites.probs[probs[i]],
...
def get_main_player_state(env_components: Env_components, list_agent, list_data, action=None):
    ...
    if env_components.winner == -1:
        ...
        if env_components.winner != -1 or env_components.env[230] == MAX_TURN_IN_ONE_GAME:
